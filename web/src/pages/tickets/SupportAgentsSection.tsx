 ...
 * usuarios en UsersPage, envuelto en el <Card>/<CardBody> del host — igual
 * que ClientDetailPage envuelve a ClientSystemsTab — así que este
 * componente no dibuja su propio contenedor con borde/fondo.
 * GET /support-agents es de cualquier usuario autenticado, pero
 * create/update/remove están detrás de @Roles('ADMIN') (ver
 * backend/src/modules/tickets/support-agents.controller.ts): un usuario
 * no-admin nunca ve los controles de edición, para no ofrecer un botón que
...
export default function SupportAgentsSection() {
  ...
  // Sin catch propio: una falla de refresco debe distinguirse de una falla
  // de escritura (ver safeReload), así que se deja propagar.
  const reload = () => supportAgentsApi.list().then(setAgents);
  ...
  // GET de refresco falla (red, 5xx transitorio), una fila puede "rebotar"
  // a su valor anterior (p. ej. un checkbox de especialidad se desmarca
  // solo) aunque el cambio quedó guardado en el servidor. Sin este aviso
  // separado del de la escritura, el usuario ve el estado divergir de lo
  // que realmente pasó y no tiene ninguna pista de por qué.
  const safeReload = async () => {
    try {
      await reload();
    } catch (e) {
      console.warn('[SupportAgentsSection] La acción se guardó, pero no se pudo refrescar la lista.', e);
            ...
            >
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          aria-label={`${SERVICE_CATEGORY_LABELS[c]} — especialidad de ${a.fullName}`}
                        />
              ...
              {isAdmin ? (
                <button
                  ...
                  disabled={busy}
                  aria-label={`${a.isActive ? 'Desactivar' : 'Activar'} técnico ${a.fullName}`}
                  style={{
                    fontSize: 11,
                    padding: '5px 8px',
                    borderRadius: 5,
              ...
              return (
                ...
                >
                  <input
                    ...
                    aria-label={`${SERVICE_CATEGORY_LABELS[c]} para el nuevo técnico`}
                  />
                  {SERVICE_CATEGORY_LABELS[c]}
                </label>
              );
